...
class CreateClientWindow(Toplevel, CenterWidgetMixin):
    ...
    def build(self):
        ...
        crear = Button(frame, text="Crear", command=self.createClient)
    ...
    def validate(self, event, index):
        ...
        if index == 0:
            valido = helpers.dniValido(valor, db.Clientes.lista)
            if valido:
...
class EditClientWindow(Toplevel, CenterWidgetMixin):
    ...
    def build(self):
        ...
        actualizar = Button(frame, text="Actualizar", command=self.editClient)
